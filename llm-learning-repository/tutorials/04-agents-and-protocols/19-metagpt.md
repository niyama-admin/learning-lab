# MetaGPT: Meta Programming for Multi-Agent Collaborative Framework - Complete Tutorial

> **Paper:** Hong et al. - arXiv:2308.00352 - [abstract](https://arxiv.org/abs/2308.00352) - [local PDF](../../papers/2308.00352.pdf)
>
...
|---|---|
| Research question | Can software-development standard operating procedures be encoded into a role-based LLM team that emits structured engineering artifacts? |
| What came before | A single conversational agent often jumps directly from a vague request to code, omitting requirements, design, interfaces, tests, and review. |
| Central contribution | MetaGPT assigns product-manager, architect, project-manager, and engineer roles, and uses SOPs plus structured messages to coordinate them. |
...
| Boundaries | Artifact presence is not artifact correctness. Fixed SOPs can make false assumptions durable, reported benchmark tasks are smaller than production systems, and security, dependency management, and maintenance remain underexplored. |
...
```mermaid
flowchart LR
  A["requirement"] --> B["product requirements"]
  B --> C["architecture and APIs"]
  C --> D["tasks and code"]
  D --> E["tests and deliverables"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **requirement** supplies the starting information. **product requirements** transforms or selects it. **architecture and APIs** is the central learned or experimental mechanism. **tasks and code** converts internal state into a task-facing result. **tests and deliverables** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use a small studio using handoff documents: the product lead writes the brief, the architect draws the plan, the manager assigns work, and engineers implement against agreed interfaces. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

A single conversational agent often jumps directly from a vague request to code, omitting requirements, design, interfaces, tests, and review. The authors ask: Can software-development standard operating procedures be encoded into a role-based LLM team that emits structured engineering artifacts? Their answer is MetaGPT assigns product-manager, architect, project-manager, and engineer roles, and uses SOPs plus structured messages to coordinate them. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **requirement**. Nothing downstream can recover information that was never present or accessible here. Next, **product requirements** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **architecture and APIs** performs the key computation or organizational step. **tasks and code** is where an internal result becomes an answer, patch, action, score, or treatment. Only **tests and deliverables** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses HumanEval, MBPP, and software-generation tasks evaluated through executable code and artifact-oriented comparisons. Decide what the system sees and what is withheld.
2. Encode or organize the input. A requirement triggers sequential artifacts such as product requirements, system design, task decomposition, APIs, code, and tests. Roles watch selected messages and act according to workflow constraints.
3. Apply the central rule. In compact notation: A role action is conditioned on profile, memory, watched messages, and SOP state rather than the latest user message alone. The symbols are unpacked in Tutorials 2 and 3.
...
The paper evaluates generated software and coding benchmarks, reporting competitive HumanEval and MBPP results and arguing that structured intermediate artifacts reduce cascading ambiguity compared with unstructured chat.

The evidence supports the paper's stated comparison, not every nearby claim. Artifact presence is not artifact correctness. Fixed SOPs can make false assumptions durable, reported benchmark tasks are smaller than production systems, and security, dependency management, and maintenance remain underexplored. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **requirement -> product requirements -> architecture and APIs -> tasks and code -> tests and deliverables**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: A role action is conditioned on profile, memory, watched messages, and SOP state rather than the latest user message alone. Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **requirement** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **product requirements** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **architecture and APIs** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **tasks and code** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **tests and deliverables** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

A requirement triggers sequential artifacts such as product requirements, system design, task decomposition, APIs, code, and tests. Roles watch selected messages and act according to workflow constraints. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is HumanEval, MBPP, and software-generation tasks evaluated through executable code and artifact-oriented comparisons. The headline evidence is: The paper evaluates generated software and coding benchmarks, reporting competitive HumanEval and MBPP results and arguing that structured intermediate artifacts reduce cascading ambiguity compared with unstructured chat.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. Artifact presence is not artifact correctness. Fixed SOPs can make false assumptions durable, reported benchmark tasks are smaller than production systems, and security, dependency management, and maintenance remain underexplored.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is A role action is conditioned on profile, memory, watched messages, and SOP state rather than the latest user message alone. Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
The paper evaluates generated software and coding benchmarks, reporting competitive HumanEval and MBPP results and arguing that structured intermediate artifacts reduce cascading ambiguity compared with unstructured chat. Reconstruct this evidence from raw units before looking at the aggregate. Specify HumanEval, MBPP, and software-generation tasks evaluated through executable code and artifact-oriented comparisons. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
Artifact presence is not artifact correctness. Fixed SOPs can make false assumptions durable, reported benchmark tasks are smaller than production systems, and security, dependency management, and maintenance remain underexplored.
...
The concrete representation path is requirement -> product requirements -> architecture and APIs -> tasks and code -> tests and deliverables. The paper's formal core can be summarized as A role action is conditioned on profile, memory, watched messages, and SOP state rather than the latest user message alone. Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: A role action is conditioned on profile, memory, watched messages, and SOP state rather than the latest user message alone. Separate what is directly optimized from what is merely evaluated. That distinction explains why success on HumanEval, MBPP, and software-generation tasks evaluated through executable code and artifact-oriented comparisons does not by itself optimize every deployment goal.
...
The paper operationalizes its task through HumanEval, MBPP, and software-generation tasks evaluated through executable code and artifact-oriented comparisons. Its pipeline - requirement -> product requirements -> architecture and APIs -> tasks and code -> tests and deliverables - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The reported evidence is The paper evaluates generated software and coding benchmarks, reporting competitive HumanEval and MBPP results and arguing that structured intermediate artifacts reduce cascading ambiguity compared with unstructured chat. These measurements support a bounded comparison under the paper's design. They do not mean statistical uncertainty was measured unless the paper reports an interval, resampling method, or model. Recompute metrics at the original experimental unit and add uncertainty appropriate to dependence in the data.
...
The operational path is requirement -> product requirements -> architecture and APIs -> tasks and code -> tests and deliverables. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: Artifact presence is not artifact correctness. Fixed SOPs can make false assumptions durable, reported benchmark tasks are smaller than production systems, and security, dependency management, and maintenance remain underexplored. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **role specialization:** distinct prompts and responsibilities
- **structured message:** typed handoff artifact
- **watch:** subscription to relevant events
- **shared environment:** common workspace/state
- **cascading hallucination:** early error propagated through later work
...
This tutorial is a structured explanation of [MetaGPT: Meta Programming for Multi-Agent Collaborative Framework](https://arxiv.org/abs/2308.00352) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive A role action is conditioned on profile, memory, watched messages, and SOP state rather than the latest user message alone, reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
