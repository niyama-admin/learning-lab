# CAMEL: Communicative Agents for Mind Exploration of Large Scale Language Model Society - Complete Tutorial
...
|---|---|
| Research question | Can role prompts sustain useful autonomous cooperation between language-model agents while reducing the need for a human to steer every turn? |
| What came before | Open-ended chat agents often drift, repeat, or require frequent human prompts to maintain a coherent task. |
| Central contribution | CAMEL introduces inception prompting and role-playing between an AI user and AI assistant, seeded by a task generated from specified roles. |
...
| Boundaries | Dialogue fluency is not verified task correctness. Both agents share model biases and can mutually reinforce errors; termination, external grounding, cost, and security remain open engineering concerns. |
...
```mermaid
flowchart LR
  A["human idea and roles"] --> B["task specifier"]
  B --> C["AI user"]
  C --> D["AI assistant"]
  D --> E["termination or result"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **human idea and roles** supplies the starting information. **task specifier** transforms or selects it. **AI user** is the central learned or experimental mechanism. **AI assistant** converts internal state into a task-facing result. **termination or result** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use two improvisers receive detailed character cards and a shared assignment, then alternate turns while a stage manager watches for loops and role confusion. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

Open-ended chat agents often drift, repeat, or require frequent human prompts to maintain a coherent task. The authors ask: Can role prompts sustain useful autonomous cooperation between language-model agents while reducing the need for a human to steer every turn? Their answer is CAMEL introduces inception prompting and role-playing between an AI user and AI assistant, seeded by a task generated from specified roles. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **human idea and roles**. Nothing downstream can recover information that was never present or accessible here. Next, **task specifier** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **AI user** performs the key computation or organizational step. **AI assistant** is where an internal result becomes an answer, patch, action, score, or treatment. Only **termination or result** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses AI Society and Code conversational datasets generated from role pairs and task prompts, plus analyzed dialogue traces. Decide what the system sees and what is withheld.
2. Encode or organize the input. A task-specifier expands a broad idea into a concrete task. Separate system prompts define complementary roles, rules, and termination behavior; agents alternate messages to pursue the task.
3. Apply the central rule. In compact notation: Each agent samples message m_t from p_theta(m_t | role prompt, task, dialogue history). The symbols are unpacked in Tutorials 2 and 3.
...
The paper presents large-scale conversational data and qualitative/quantitative analyses of role-playing, including task completion, consistency, and common failure modes such as role flipping, assistant repetition, and conversation loops.

The evidence supports the paper's stated comparison, not every nearby claim. Dialogue fluency is not verified task correctness. Both agents share model biases and can mutually reinforce errors; termination, external grounding, cost, and security remain open engineering concerns. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **human idea and roles -> task specifier -> AI user -> AI assistant -> termination or result**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: Each agent samples message m_t from p_theta(m_t | role prompt, task, dialogue history). Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **human idea and roles** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **task specifier** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **AI user** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **AI assistant** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **termination or result** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

A task-specifier expands a broad idea into a concrete task. Separate system prompts define complementary roles, rules, and termination behavior; agents alternate messages to pursue the task. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is AI Society and Code conversational datasets generated from role pairs and task prompts, plus analyzed dialogue traces. The headline evidence is: The paper presents large-scale conversational data and qualitative/quantitative analyses of role-playing, including task completion, consistency, and common failure modes such as role flipping, assistant repetition, and conversation loops.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. Dialogue fluency is not verified task correctness. Both agents share model biases and can mutually reinforce errors; termination, external grounding, cost, and security remain open engineering concerns.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is Each agent samples message m_t from p_theta(m_t | role prompt, task, dialogue history). Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
The paper presents large-scale conversational data and qualitative/quantitative analyses of role-playing, including task completion, consistency, and common failure modes such as role flipping, assistant repetition, and conversation loops. Reconstruct this evidence from raw units before looking at the aggregate. Specify AI Society and Code conversational datasets generated from role pairs and task prompts, plus analyzed dialogue traces. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
Dialogue fluency is not verified task correctness. Both agents share model biases and can mutually reinforce errors; termination, external grounding, cost, and security remain open engineering concerns.
...
The concrete representation path is human idea and roles -> task specifier -> AI user -> AI assistant -> termination or result. The paper's formal core can be summarized as Each agent samples message m_t from p_theta(m_t | role prompt, task, dialogue history). Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: Each agent samples message m_t from p_theta(m_t | role prompt, task, dialogue history). Separate what is directly optimized from what is merely evaluated. That distinction explains why success on AI Society and Code conversational datasets generated from role pairs and task prompts, plus analyzed dialogue traces does not by itself optimize every deployment goal.
...
The paper operationalizes its task through AI Society and Code conversational datasets generated from role pairs and task prompts, plus analyzed dialogue traces. Its pipeline - human idea and roles -> task specifier -> AI user -> AI assistant -> termination or result - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The reported evidence is The paper presents large-scale conversational data and qualitative/quantitative analyses of role-playing, including task completion, consistency, and common failure modes such as role flipping, assistant repetition, and conversation loops. These measurements support a bounded comparison under the paper's design. They do not mean statistical uncertainty was measured unless the paper reports an interval, resampling method, or model. Recompute metrics at the original experimental unit and add uncertainty appropriate to dependence in the data.
...
The operational path is human idea and roles -> task specifier -> AI user -> AI assistant -> termination or result. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: Dialogue fluency is not verified task correctness. Both agents share model biases and can mutually reinforce errors; termination, external grounding, cost, and security remain open engineering concerns. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **inception prompt:** detailed role and behavior instruction
- **role-playing:** agents act under complementary identities
- **task specifier:** component that narrows an idea
- **role flipping:** agents exchange intended roles
- **conversation loop:** repeated unproductive turns
- **society of mind:** intelligence framed as interacting specialists
...
This tutorial is a structured explanation of [CAMEL: Communicative Agents for Mind Exploration of Large Scale Language Model Society](https://arxiv.org/abs/2303.17760) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive Each agent samples message m_t from p_theta(m_t | role prompt, task, dialogue history), reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
