# A Survey of Agent Interoperability Protocols: MCP, ACP, A2A, and ANP - Complete Tutorial
...
|---|---|
| Research question | How do emerging agent interoperability protocols differ in discovery, communication, identity, capability exposure, and trust? |
| What came before | Agent frameworks used custom integrations, while tools, agents, and remote services lacked a shared vocabulary for interoperation. |
| Central contribution | The survey compares MCP, ACP, A2A, and ANP as complementary or competing protocol approaches and proposes a common evaluation taxonomy. |
...
| Boundaries | The May 2025 snapshot can become outdated as specifications evolve. Stated features are not proof of interoperable implementations, secure defaults, scalability, or organizational adoption. |
...
```mermaid
flowchart LR
  A["agent or host"] --> B["discovery"]
  B --> C["protocol message"]
  C --> D["tool or remote agent"]
  D --> E["result and state"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **agent or host** supplies the starting information. **discovery** transforms or selects it. **protocol message** is the central learned or experimental mechanism. **tool or remote agent** converts internal state into a task-facing result. **result and state** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use comparing USB for attaching tools, courier protocols for delegating jobs, and passports/directories for discovering and trusting independent workers. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

Agent frameworks used custom integrations, while tools, agents, and remote services lacked a shared vocabulary for interoperation. The authors ask: How do emerging agent interoperability protocols differ in discovery, communication, identity, capability exposure, and trust? Their answer is The survey compares MCP, ACP, A2A, and ANP as complementary or competing protocol approaches and proposes a common evaluation taxonomy. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **agent or host**. Nothing downstream can recover information that was never present or accessible here. Next, **discovery** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **protocol message** performs the key computation or organizational step. **tool or remote agent** is where an internal result becomes an answer, patch, action, score, or treatment. Only **result and state** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses Primary protocol specifications, implementations, and ecosystem examples for MCP, ACP, A2A, and ANP available to the survey authors. Decide what the system sees and what is withheld.
2. Encode or organize the input. Protocol documents and implementations are compared across architecture, message transport, capability discovery, state, security, governance, and ecosystem maturity.
3. Apply the central rule. In compact notation: Interoperability is a compatibility relation over transport, message schema, lifecycle, identity, authorization, and error semantics - not merely valid JSON. The symbols are unpacked in Tutorials 2 and 3.
...
This is a standards survey, so evidence is feature comparison and case analysis rather than model-accuracy experiments. It distinguishes tool/context connectivity from agent-to-agent task delegation and decentralized identity approaches.

The evidence supports the paper's stated comparison, not every nearby claim. The May 2025 snapshot can become outdated as specifications evolve. Stated features are not proof of interoperable implementations, secure defaults, scalability, or organizational adoption. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **agent or host -> discovery -> protocol message -> tool or remote agent -> result and state**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: Interoperability is a compatibility relation over transport, message schema, lifecycle, identity, authorization, and error semantics - not merely valid JSON. Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **agent or host** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **discovery** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **protocol message** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **tool or remote agent** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **result and state** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

Protocol documents and implementations are compared across architecture, message transport, capability discovery, state, security, governance, and ecosystem maturity. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is Primary protocol specifications, implementations, and ecosystem examples for MCP, ACP, A2A, and ANP available to the survey authors. The headline evidence is: This is a standards survey, so evidence is feature comparison and case analysis rather than model-accuracy experiments. It distinguishes tool/context connectivity from agent-to-agent task delegation and decentralized identity approaches.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. The May 2025 snapshot can become outdated as specifications evolve. Stated features are not proof of interoperable implementations, secure defaults, scalability, or organizational adoption.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is Interoperability is a compatibility relation over transport, message schema, lifecycle, identity, authorization, and error semantics - not merely valid JSON. Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
This is a standards survey, so evidence is feature comparison and case analysis rather than model-accuracy experiments. It distinguishes tool/context connectivity from agent-to-agent task delegation and decentralized identity approaches. Reconstruct this evidence from raw units before looking at the aggregate. Specify Primary protocol specifications, implementations, and ecosystem examples for MCP, ACP, A2A, and ANP available to the survey authors. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
The May 2025 snapshot can become outdated as specifications evolve. Stated features are not proof of interoperable implementations, secure defaults, scalability, or organizational adoption.
...
The concrete representation path is agent or host -> discovery -> protocol message -> tool or remote agent -> result and state. The paper's formal core can be summarized as Interoperability is a compatibility relation over transport, message schema, lifecycle, identity, authorization, and error semantics - not merely valid JSON. Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: Interoperability is a compatibility relation over transport, message schema, lifecycle, identity, authorization, and error semantics - not merely valid JSON. Separate what is directly optimized from what is merely evaluated. That distinction explains why success on Primary protocol specifications, implementations, and ecosystem examples for MCP, ACP, A2A, and ANP available to the survey authors does not by itself optimize every deployment goal.
...
The paper operationalizes its task through Primary protocol specifications, implementations, and ecosystem examples for MCP, ACP, A2A, and ANP available to the survey authors. Its pipeline - agent or host -> discovery -> protocol message -> tool or remote agent -> result and state - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The reported evidence is This is a standards survey, so evidence is feature comparison and case analysis rather than model-accuracy experiments. It distinguishes tool/context connectivity from agent-to-agent task delegation and decentralized identity approaches. These measurements support a bounded comparison under the paper's design. They do not mean statistical uncertainty was measured unless the paper reports an interval, resampling method, or model. Recompute metrics at the original experimental unit and add uncertainty appropriate to dependence in the data.
...
The operational path is agent or host -> discovery -> protocol message -> tool or remote agent -> result and state. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: The May 2025 snapshot can become outdated as specifications evolve. Stated features are not proof of interoperable implementations, secure defaults, scalability, or organizational adoption. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **capability discovery:** learning what a peer offers
- **transport:** mechanism carrying protocol messages
...
This tutorial is a structured explanation of [A Survey of Agent Interoperability Protocols: MCP, ACP, A2A, and ANP](https://arxiv.org/abs/2505.02279) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive Interoperability is a compatibility relation over transport, message schema, lifecycle, identity, authorization, and error semantics - not merely valid JSON, reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
