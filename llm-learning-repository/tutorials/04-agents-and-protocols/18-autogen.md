# AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation - Complete Tutorial
...
|---|---|
| Research question | Can a general conversation abstraction compose LLMs, tools, code execution, and humans into flexible multi-agent applications? |
| What came before | Agent systems were often built as rigid, application-specific control flows that were difficult to reuse or extend. |
| Central contribution | AutoGen represents components as conversable agents with customizable reply behavior and supports automated or human-in-the-loop multi-agent conversations. |
...
| Boundaries | Framework flexibility does not ensure correctness. Unbounded conversations, unsafe code, prompt injection, correlated agents, nondeterminism, and unclear responsibility require explicit controls and evaluation. |
...
```mermaid
flowchart LR
  A["message"] --> B["conversable agent"]
  B --> C["LLM tool or human reply"]
  C --> D["next recipient"]
  D --> E["termination condition"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **message** supplies the starting information. **conversable agent** transforms or selects it. **LLM tool or human reply** is the central learned or experimental mechanism. **next recipient** converts internal state into a task-facing result. **termination condition** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use a switchboard where specialists can call one another, run approved machinery, or ask a supervisor, while configurable rules decide who answers next. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

Agent systems were often built as rigid, application-specific control flows that were difficult to reuse or extend. The authors ask: Can a general conversation abstraction compose LLMs, tools, code execution, and humans into flexible multi-agent applications? Their answer is AutoGen represents components as conversable agents with customizable reply behavior and supports automated or human-in-the-loop multi-agent conversations. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **message**. Nothing downstream can recover information that was never present or accessible here. Next, **conversable agent** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **LLM tool or human reply** performs the key computation or organizational step. **next recipient** is where an internal result becomes an answer, patch, action, score, or treatment. Only **termination condition** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses Demonstration applications and task suites spanning mathematics, coding, retrieval, decision optimization, and interactive games. Decide what the system sees and what is withheld.
2. Encode or organize the input. Agents send messages, register reply functions, call LLMs or tools, execute code, and apply termination conditions. Conversation patterns can be two-agent, group, nested, or customized.
3. Apply the central rule. In compact notation: Agent i implements a reply policy pi_i(message history, local state) -> reply, action, recipient, or terminate. The symbols are unpacked in Tutorials 2 and 3.
...
The paper demonstrates applications in math problem solving, retrieval-augmented chat, decision support, coding, and conversational chess, showing how different orchestration patterns improve capability or reduce manual integration.

The evidence supports the paper's stated comparison, not every nearby claim. Framework flexibility does not ensure correctness. Unbounded conversations, unsafe code, prompt injection, correlated agents, nondeterminism, and unclear responsibility require explicit controls and evaluation. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **message -> conversable agent -> LLM tool or human reply -> next recipient -> termination condition**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: Agent i implements a reply policy pi_i(message history, local state) -> reply, action, recipient, or terminate. Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **message** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **conversable agent** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **LLM tool or human reply** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **next recipient** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **termination condition** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

Agents send messages, register reply functions, call LLMs or tools, execute code, and apply termination conditions. Conversation patterns can be two-agent, group, nested, or customized. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is Demonstration applications and task suites spanning mathematics, coding, retrieval, decision optimization, and interactive games. The headline evidence is: The paper demonstrates applications in math problem solving, retrieval-augmented chat, decision support, coding, and conversational chess, showing how different orchestration patterns improve capability or reduce manual integration.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. Framework flexibility does not ensure correctness. Unbounded conversations, unsafe code, prompt injection, correlated agents, nondeterminism, and unclear responsibility require explicit controls and evaluation.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is Agent i implements a reply policy pi_i(message history, local state) -> reply, action, recipient, or terminate. Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
The paper demonstrates applications in math problem solving, retrieval-augmented chat, decision support, coding, and conversational chess, showing how different orchestration patterns improve capability or reduce manual integration. Reconstruct this evidence from raw units before looking at the aggregate. Specify Demonstration applications and task suites spanning mathematics, coding, retrieval, decision optimization, and interactive games. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
Framework flexibility does not ensure correctness. Unbounded conversations, unsafe code, prompt injection, correlated agents, nondeterminism, and unclear responsibility require explicit controls and evaluation.
...
The concrete representation path is message -> conversable agent -> LLM tool or human reply -> next recipient -> termination condition. The paper's formal core can be summarized as Agent i implements a reply policy pi_i(message history, local state) -> reply, action, recipient, or terminate. Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: Agent i implements a reply policy pi_i(message history, local state) -> reply, action, recipient, or terminate. Separate what is directly optimized from what is merely evaluated. That distinction explains why success on Demonstration applications and task suites spanning mathematics, coding, retrieval, decision optimization, and interactive games does not by itself optimize every deployment goal.
...
The paper operationalizes its task through Demonstration applications and task suites spanning mathematics, coding, retrieval, decision optimization, and interactive games. Its pipeline - message -> conversable agent -> LLM tool or human reply -> next recipient -> termination condition - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The reported evidence is The paper demonstrates applications in math problem solving, retrieval-augmented chat, decision support, coding, and conversational chess, showing how different orchestration patterns improve capability or reduce manual integration. These measurements support a bounded comparison under the paper's design. They do not mean statistical uncertainty was measured unless the paper reports an interval, resampling method, or model. Recompute metrics at the original experimental unit and add uncertainty appropriate to dependence in the data.
...
The operational path is message -> conversable agent -> LLM tool or human reply -> next recipient -> termination condition. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: Framework flexibility does not ensure correctness. Unbounded conversations, unsafe code, prompt injection, correlated agents, nondeterminism, and unclear responsibility require explicit controls and evaluation. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **conversable agent:** message-driven component
- **reply function:** handler producing the next response
- **human-in-the-loop:** explicit human participation
- **code executor:** component running generated code
- **group chat:** multiple agents sharing a thread
- **termination condition:** rule ending interaction
...
This tutorial is a structured explanation of [AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation](https://arxiv.org/abs/2308.08155) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive Agent i implements a reply policy pi_i(message history, local state) -> reply, action, recipient, or terminate, reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
