# Large Language Model based Multi-Agents: A Survey of Progress and Challenges - Complete Tutorial
...
|---|---|
| Research question | What design dimensions, applications, evaluation methods, and open challenges characterize LLM-based multi-agent systems? |
| What came before | Rapidly growing agent work used inconsistent terminology and fragmented architectures, making systems difficult to compare. |
| Central contribution | The survey organizes multi-agent research around agent profiling, perception, self-action, mutual interaction, evolution, environments, applications, and evaluation. |
...
| Boundaries | Survey coverage ages quickly and inherits weaknesses of cited studies. Categories can overlap, demonstrations are heterogeneous, and few evaluations isolate the causal value of multiple agents over equivalent single-agent compute. |
...
```mermaid
flowchart LR
  A["agent profiles"] --> B["perception and memory"]
  B --> C["actions"]
  C --> D["interaction topology"]
  D --> E["environment and evaluation"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **agent profiles** supplies the starting information. **perception and memory** transforms or selects it. **actions** is the central learned or experimental mechanism. **interaction topology** converts internal state into a task-facing result. **environment and evaluation** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use a field guide classifying teams by member roles, senses, communication rules, learning, workplace, and scorecards rather than declaring one team universally best. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

Rapidly growing agent work used inconsistent terminology and fragmented architectures, making systems difficult to compare. The authors ask: What design dimensions, applications, evaluation methods, and open challenges characterize LLM-based multi-agent systems? Their answer is The survey organizes multi-agent research around agent profiling, perception, self-action, mutual interaction, evolution, environments, applications, and evaluation. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **agent profiles**. Nothing downstream can recover information that was never present or accessible here. Next, **perception and memory** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **actions** performs the key computation or organizational step. **interaction topology** is where an internal result becomes an answer, patch, action, score, or treatment. Only **environment and evaluation** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses A literature corpus of LLM multi-agent systems and applications rather than a single benchmark dataset. Decide what the system sees and what is withheld.
2. Encode or organize the input. It reviews representative systems and builds taxonomies for agent construction, communication topology, cooperation/competition, feedback, memory, and capability evolution.
3. Apply the central rule. In compact notation: A multi-agent system can be modeled as policies pi_1...pi_n acting on local observations with messages that alter each agent's information state. The symbols are unpacked in Tutorials 2 and 3.
...
The contribution is synthesis rather than one controlled experiment. Cross-paper examples show promise in problem solving, simulation, software engineering, recommendation, and social-science settings, alongside recurring coordination failures.

The evidence supports the paper's stated comparison, not every nearby claim. Survey coverage ages quickly and inherits weaknesses of cited studies. Categories can overlap, demonstrations are heterogeneous, and few evaluations isolate the causal value of multiple agents over equivalent single-agent compute. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **agent profiles -> perception and memory -> actions -> interaction topology -> environment and evaluation**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: A multi-agent system can be modeled as policies pi_1...pi_n acting on local observations with messages that alter each agent's information state. Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **agent profiles** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **perception and memory** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **actions** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **interaction topology** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **environment and evaluation** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

It reviews representative systems and builds taxonomies for agent construction, communication topology, cooperation/competition, feedback, memory, and capability evolution. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is A literature corpus of LLM multi-agent systems and applications rather than a single benchmark dataset. The headline evidence is: The contribution is synthesis rather than one controlled experiment. Cross-paper examples show promise in problem solving, simulation, software engineering, recommendation, and social-science settings, alongside recurring coordination failures.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. Survey coverage ages quickly and inherits weaknesses of cited studies. Categories can overlap, demonstrations are heterogeneous, and few evaluations isolate the causal value of multiple agents over equivalent single-agent compute.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is A multi-agent system can be modeled as policies pi_1...pi_n acting on local observations with messages that alter each agent's information state. Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
The contribution is synthesis rather than one controlled experiment. Cross-paper examples show promise in problem solving, simulation, software engineering, recommendation, and social-science settings, alongside recurring coordination failures. Reconstruct this evidence from raw units before looking at the aggregate. Specify A literature corpus of LLM multi-agent systems and applications rather than a single benchmark dataset. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
Survey coverage ages quickly and inherits weaknesses of cited studies. Categories can overlap, demonstrations are heterogeneous, and few evaluations isolate the causal value of multiple agents over equivalent single-agent compute.
...
The concrete representation path is agent profiles -> perception and memory -> actions -> interaction topology -> environment and evaluation. The paper's formal core can be summarized as A multi-agent system can be modeled as policies pi_1...pi_n acting on local observations with messages that alter each agent's information state. Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: A multi-agent system can be modeled as policies pi_1...pi_n acting on local observations with messages that alter each agent's information state. Separate what is directly optimized from what is merely evaluated. That distinction explains why success on A literature corpus of LLM multi-agent systems and applications rather than a single benchmark dataset does not by itself optimize every deployment goal.
...
The paper operationalizes its task through A literature corpus of LLM multi-agent systems and applications rather than a single benchmark dataset. Its pipeline - agent profiles -> perception and memory -> actions -> interaction topology -> environment and evaluation - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The reported evidence is The contribution is synthesis rather than one controlled experiment. Cross-paper examples show promise in problem solving, simulation, software engineering, recommendation, and social-science settings, alongside recurring coordination failures. These measurements support a bounded comparison under the paper's design. They do not mean statistical uncertainty was measured unless the paper reports an interval, resampling method, or model. Recompute metrics at the original experimental unit and add uncertainty appropriate to dependence in the data.
...
The operational path is agent profiles -> perception and memory -> actions -> interaction topology -> environment and evaluation. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: Survey coverage ages quickly and inherits weaknesses of cited studies. Categories can overlap, demonstrations are heterogeneous, and few evaluations isolate the causal value of multiple agents over equivalent single-agent compute. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **profile:** role and capability definition
- **perception:** intake from environment or peers
- **topology:** who can communicate with whom
- **emergence:** group behavior not explicit in one agent
- **cooperation:** aligned objectives
- **competition:** partially conflicting objectives
...
This tutorial is a structured explanation of [Large Language Model based Multi-Agents: A Survey of Progress and Challenges](https://arxiv.org/abs/2402.01680) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive A multi-agent system can be modeled as policies pi_1...pi_n acting on local observations with messages that alter each agent's information state, reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
