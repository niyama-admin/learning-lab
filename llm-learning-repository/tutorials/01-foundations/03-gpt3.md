# Language Models are Few-Shot Learners - Complete Tutorial

> **Paper:** Brown et al. - arXiv:2005.14165 - [abstract](https://arxiv.org/abs/2005.14165) - [local PDF](../../papers/2005.14165.pdf)
>
...
|---|---|
| Research question | How far can task performance emerge from scale and natural-language prompting without gradient updates for each downstream task? |
| What came before | The dominant transfer recipe pretrained a model and then fine-tuned it separately for each labeled dataset. |
| Central contribution | GPT-3 scales an autoregressive Transformer to 175B parameters and evaluates zero-shot, one-shot, and few-shot in-context learning across many tasks. |
...
| Boundaries | A 175B model is expensive, prompt-sensitive, and trained on broad web data. In-context pattern matching is not proof of human-like learning, and benchmark performance does not guarantee truthfulness or safe business behavior. |
...
```mermaid
flowchart LR
  A["large text mixture"] --> B["autoregressive pretraining"]
  B --> C["prompt plus examples"]
  C --> D["next-token inference"]
  D --> E["task answer"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **large text mixture** supplies the starting information. **autoregressive pretraining** transforms or selects it. **prompt plus examples** is the central learned or experimental mechanism. **next-token inference** converts internal state into a task-facing result. **task answer** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use a widely read improviser who is shown a few examples on the cue card before performing a new scene. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

The dominant transfer recipe pretrained a model and then fine-tuned it separately for each labeled dataset. The authors ask: How far can task performance emerge from scale and natural-language prompting without gradient updates for each downstream task? Their answer is GPT-3 scales an autoregressive Transformer to 175B parameters and evaluates zero-shot, one-shot, and few-shot in-context learning across many tasks. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **large text mixture**. Nothing downstream can recover information that was never present or accessible here. Next, **autoregressive pretraining** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **prompt plus examples** performs the key computation or organizational step. **next-token inference** is where an internal result becomes an answer, patch, action, score, or treatment. Only **task answer** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses Common Crawl, WebText2, Books1/2, and Wikipedia for training; a broad suite including LAMBADA, SuperGLUE, TriviaQA, translation, arithmetic, and cloze tasks. Decide what the system sees and what is withheld.
2. Encode or organize the input. A prompt contains instructions or demonstrations followed by a query. The model predicts continuation tokens; no weight update occurs during evaluation. The study compares model sizes from 125M through 175B to expose scaling trends.
3. Apply the central rule. In compact notation: L(theta) = -sum_t log p_theta(x_t | x_<t); in-context examples change x_<t but not theta. The symbols are unpacked in Tutorials 2 and 3.
...
Performance generally improves with parameter count and demonstrations, with striking gains on language modeling, question answering, translation, and synthetic reasoning. The paper also documents weak spots such as some reading-comprehension and comparison tasks, and analyzes benchmark contamination.

The evidence supports the paper's stated comparison, not every nearby claim. A 175B model is expensive, prompt-sensitive, and trained on broad web data. In-context pattern matching is not proof of human-like learning, and benchmark performance does not guarantee truthfulness or safe business behavior. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **large text mixture -> autoregressive pretraining -> prompt plus examples -> next-token inference -> task answer**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: L(theta) = -sum_t log p_theta(x_t | x_<t); in-context examples change x_<t but not theta. Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **large text mixture** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **autoregressive pretraining** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **prompt plus examples** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **next-token inference** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **task answer** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

A prompt contains instructions or demonstrations followed by a query. The model predicts continuation tokens; no weight update occurs during evaluation. The study compares model sizes from 125M through 175B to expose scaling trends. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is Common Crawl, WebText2, Books1/2, and Wikipedia for training; a broad suite including LAMBADA, SuperGLUE, TriviaQA, translation, arithmetic, and cloze tasks. The headline evidence is: Performance generally improves with parameter count and demonstrations, with striking gains on language modeling, question answering, translation, and synthetic reasoning. The paper also documents weak spots such as some reading-comprehension and comparison tasks, and analyzes benchmark contamination.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. A 175B model is expensive, prompt-sensitive, and trained on broad web data. In-context pattern matching is not proof of human-like learning, and benchmark performance does not guarantee truthfulness or safe business behavior.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is L(theta) = -sum_t log p_theta(x_t | x_<t); in-context examples change x_<t but not theta. Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
Performance generally improves with parameter count and demonstrations, with striking gains on language modeling, question answering, translation, and synthetic reasoning. The paper also documents weak spots such as some reading-comprehension and comparison tasks, and analyzes benchmark contamination. Reconstruct this evidence from raw units before looking at the aggregate. Specify Common Crawl, WebText2, Books1/2, and Wikipedia for training; a broad suite including LAMBADA, SuperGLUE, TriviaQA, translation, arithmetic, and cloze tasks. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
A 175B model is expensive, prompt-sensitive, and trained on broad web data. In-context pattern matching is not proof of human-like learning, and benchmark performance does not guarantee truthfulness or safe business behavior.
...
The concrete representation path is large text mixture -> autoregressive pretraining -> prompt plus examples -> next-token inference -> task answer. The paper's formal core can be summarized as L(theta) = -sum_t log p_theta(x_t | x_<t); in-context examples change x_<t but not theta. Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: L(theta) = -sum_t log p_theta(x_t | x_<t); in-context examples change x_<t but not theta. Separate what is directly optimized from what is merely evaluated. That distinction explains why success on Common Crawl, WebText2, Books1/2, and Wikipedia for training; a broad suite including LAMBADA, SuperGLUE, TriviaQA, translation, arithmetic, and cloze tasks does not by itself optimize every deployment goal.
...
The paper operationalizes its task through Common Crawl, WebText2, Books1/2, and Wikipedia for training; a broad suite including LAMBADA, SuperGLUE, TriviaQA, translation, arithmetic, and cloze tasks. Its pipeline - large text mixture -> autoregressive pretraining -> prompt plus examples -> next-token inference -> task answer - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The operational path is large text mixture -> autoregressive pretraining -> prompt plus examples -> next-token inference -> task answer. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: A 175B model is expensive, prompt-sensitive, and trained on broad web data. In-context pattern matching is not proof of human-like learning, and benchmark performance does not guarantee truthfulness or safe business behavior. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **in-context learning:** adapting behavior through prompt context
- **autoregressive:** predicting the next token from earlier tokens
- **contamination:** test material appearing in training data
...
This tutorial is a structured explanation of [Language Models are Few-Shot Learners](https://arxiv.org/abs/2005.14165) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive L(theta) = -sum_t log p_theta(x_t | x_<t); in-context examples change x_<t but not theta, reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
