...
>
> **Why it is here:** autoregressive scaling and in-context learning. Read after the preceding items in the roadmap.

## One-picture map

```mermaid
flowchart LR
  A["instructions + examples"] --> B["next-token model"]
  B["next-token model"] --> C["continuation"]
  C["continuation"] --> D["parsed task answer"]
```
