...
>
> **Why it is here:** interleaving thought, tool actions, and observations. Read after the preceding items in the roadmap.
...
flowchart LR
  A["goal"] --> B["reason"]
  B["reason"] --> C["action"]
  C["action"] --> D["observation"]
  D["observation"] --> E["reason"]
  E["reason"] --> F["..."]
  F["..."] --> G["answer"]
```
...
ReAct alternates between making a plan, using a tool, reading the result, and deciding what to do next.
...
Define typed tools, bounded loops, timeouts, permissions, observation sanitization, and outcome traces; evaluate final success and unsafe attempts.
...
Model the loop as a partially observable decision process; analyze credit assignment, tool-error recovery, prompt leakage, and planning versus reactive control.
