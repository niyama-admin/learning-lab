# ReAct: Synergizing Reasoning and Acting in Language Models - Complete Tutorial

> **Paper:** Yao et al. - arXiv:2210.03629 - [abstract](https://arxiv.org/abs/2210.03629) - [local PDF](../../papers/2210.03629.pdf)
>
...
|---|---|
| Research question | Can an LLM interleave verbal reasoning with tool actions so that thought guides information gathering and observations correct thought? |
| What came before | Reasoning-only prompting could hallucinate facts; action-only agents lacked an explicit scratchpad for planning and exception handling. |
| Central contribution | ReAct uses trajectories of Thought, Action, Observation steps followed by an answer. |
...
| Boundaries | Thought text can still be unfaithful, tools can fail, and long trajectories compound error and cost. Tool permissions, prompt injection, termination, and audit controls are application responsibilities. |
...
```mermaid
flowchart LR
  A["question or goal"] --> B["thought"]
  B --> C["tool action"]
  C --> D["observation"]
  D --> E["next thought or answer"]
  E -. feedback and audit .-> A
```

Read the arrows as dependencies, not decoration. **question or goal** supplies the starting information. **thought** transforms or selects it. **tool action** is the central learned or experimental mechanism. **observation** converts internal state into a task-facing result. **next thought or answer** is what can finally be measured. The dotted feedback arrow is modern operational advice: observe failures and revise data, prompts, components, or policy. It is not necessarily part of the original experiment.

## Tutorial 1 - Intuitive understanding

Use a field technician who alternates between writing the next hypothesis, operating an instrument, recording the reading, and revising the plan. The point of the analogy is not that a model thinks like a person. It separates three ideas that are easy to blur: prior material, a procedure that uses that material, and evidence that the resulting behavior improved.

### The problem in ordinary language

Reasoning-only prompting could hallucinate facts; action-only agents lacked an explicit scratchpad for planning and exception handling. The authors ask: Can an LLM interleave verbal reasoning with tool actions so that thought guides information gathering and observations correct thought? Their answer is ReAct uses trajectories of Thought, Action, Observation steps followed by an answer. This is important because it identifies an intervention one can compare, rather than treating “more intelligence” as an explanation.

Walk through one case. First comes **question or goal**. Nothing downstream can recover information that was never present or accessible here. Next, **thought** determines what is emphasized, hidden, retrieved, assigned, or compared. Then **tool action** performs the key computation or organizational step. **observation** is where an internal result becomes an answer, patch, action, score, or treatment. Only **next thought or answer** is directly visible to evaluation.
...
1. Define the task boundary. The paper uses HotpotQA and FEVER for knowledge tasks; ALFWorld and WebShop for interactive decision-making. Decide what the system sees and what is withheld.
2. Encode or organize the input. Few-shot trajectories teach the model an action grammar. On QA tasks actions call a Wikipedia interface; on interactive tasks they manipulate an environment. Returned observations re-enter context.
3. Apply the central rule. In compact notation: At step t the policy samples a_t from pi_theta(a_t | x, thought_<t, action_<t, observation_<t). The symbols are unpacked in Tutorials 2 and 3.
...
ReAct improves interpretability and often outperforms action-only or chain-of-thought baselines. On ALFWorld and WebShop the paper reports absolute success-rate gains of 34 and 10 percentage points over imitation/reinforcement-learning baselines.

The evidence supports the paper's stated comparison, not every nearby claim. Thought text can still be unfaithful, tools can fail, and long trajectories compound error and cost. Tool permissions, prompt injection, termination, and audit controls are application responsibilities. A useful reading therefore keeps four columns: claim, operational measure, observed comparison, and remaining alternative explanations. If a number rises, ask whether the input, data split, compute, prompt, tool access, or human population also changed.
...
The implementation contract is **question or goal -> thought -> tool action -> observation -> next thought or answer**. Treat each arrow as an interface with typed input, output, error, latency, and provenance. The paper's core mechanism is summarized by: At step t the policy samples a_t from pi_theta(a_t | x, thought_<t, action_<t, observation_<t). Symbols such as theta denote learned parameters; x denotes observed input; y denotes an output or target; probabilities are conditional on the information shown after the vertical bar. Paper-specific names are defined in the glossary.
...
1. Materialize **question or goal** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
2. Materialize **thought** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
3. Materialize **tool action** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
4. Materialize **observation** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.
5. Materialize **next thought or answer** as a versioned artifact or logged event. Define its schema, owner, failure states, and acceptance check.

Few-shot trajectories teach the model an action grammar. On QA tasks actions call a Wikipedia interface; on interactive tasks they manipulate an environment. Returned observations re-enter context. For a faithful small-scale implementation, resist substituting a modern component until the original behavior is reproduced. Pin the dataset snapshot, tokenizer or parser, model identifier, prompt/adaptation format, random seeds, decoding parameters, metric implementation, and environment image. Store raw predictions as well as aggregate scores.
...
The study's evaluation material is HotpotQA and FEVER for knowledge tasks; ALFWorld and WebShop for interactive decision-making. The headline evidence is: ReAct improves interpretability and often outperforms action-only or chain-of-thought baselines. On ALFWorld and WebShop the paper reports absolute success-rate gains of 34 and 10 percentage points over imitation/reinforcement-learning baselines.
...
Operational acceptance needs more than the paper score. Define quality, cost per accepted outcome, median and tail latency, error recovery, privacy exposure, and human override. Use a shadow deployment first. Sample failures by category, not only at random, because rare severe failures may disappear in averages. Thought text can still be unfaithful, tools can fail, and long trajectories compound error and cost. Tool permissions, prompt injection, termination, and audit controls are application responsibilities.
...
The formal object is not “an intelligent system”; it is a conditional mechanism or estimator operating under a design. The central expression is At step t the policy samples a_t from pi_theta(a_t | x, thought_<t, action_<t, observation_<t). Derive it by naming each random variable or matrix, its domain and shape, what is observed, what is learned, and what is marginalized or compared. Then identify which term encodes the paper's intervention.
...
ReAct improves interpretability and often outperforms action-only or chain-of-thought baselines. On ALFWorld and WebShop the paper reports absolute success-rate gains of 34 and 10 percentage points over imitation/reinforcement-learning baselines. Reconstruct this evidence from raw units before looking at the aggregate. Specify HotpotQA and FEVER for knowledge tasks; ALFWorld and WebShop for interactive decision-making. Record exclusions, missing outputs, retries, decoding samples, human adjudication, and whether observations are independent. Recreate the main table with one row per system or group and columns for configuration, compute/tool access, sample count, metric, uncertainty, and source location.
...
Thought text can still be unfaithful, tools can fail, and long trajectories compound error and cost. Tool permissions, prompt injection, termination, and audit controls are application responsibilities.
...
The concrete representation path is question or goal -> thought -> tool action -> observation -> next thought or answer. The paper's formal core can be summarized as At step t the policy samples a_t from pi_theta(a_t | x, thought_<t, action_<t, observation_<t). Identify which objects are vectors, matrices, scalar scores, discrete choices, or observed outcomes before manipulating the notation.
...
For this paper, the central objective or estimator is: At step t the policy samples a_t from pi_theta(a_t | x, thought_<t, action_<t, observation_<t). Separate what is directly optimized from what is merely evaluated. That distinction explains why success on HotpotQA and FEVER for knowledge tasks; ALFWorld and WebShop for interactive decision-making does not by itself optimize every deployment goal.
...
The paper operationalizes its task through HotpotQA and FEVER for knowledge tasks; ALFWorld and WebShop for interactive decision-making. Its pipeline - question or goal -> thought -> tool action -> observation -> next thought or answer - determines what information is available and what counts as an answer. A reproduction must preserve those boundaries before comparing scores.
...
The reported evidence is ReAct improves interpretability and often outperforms action-only or chain-of-thought baselines. On ALFWorld and WebShop the paper reports absolute success-rate gains of 34 and 10 percentage points over imitation/reinforcement-learning baselines. These measurements support a bounded comparison under the paper's design. They do not mean statistical uncertainty was measured unless the paper reports an interval, resampling method, or model. Recompute metrics at the original experimental unit and add uncertainty appropriate to dependence in the data.
...
The operational path is question or goal -> thought -> tool action -> observation -> next thought or answer. Profile every stage rather than attributing all cost to the language model. Preserve the paper's configuration for faithful reproduction, then separately test modern batching, caching, indexing, quantization, or parallelism.
...
The paper-specific limitations are: Thought text can still be unfaithful, tools can fail, and long trajectories compound error and cost. Tool permissions, prompt injection, termination, and audit controls are application responsibilities. These are not erased by code release or benchmark gains. A deployment review should map users, data boundaries, failure impact, human override, access control, monitoring, and rollback before increasing autonomy.
...
- **trajectory:** ordered interaction history
- **Thought:** textual scratchpad step
- **Action:** structured tool command
- **Observation:** tool/environment return
- **sparse reasoning:** reasoning only when needed
- **grounding:** tying output to external evidence or state
...
This tutorial is a structured explanation of [ReAct: Synergizing Reasoning and Acting in Language Models](https://arxiv.org/abs/2210.03629) grounded in the repository's downloaded PDF. It paraphrases rather than reproduces the paper. Numerical claims should be checked against the original tables and appendices before citation, procurement, or deployment. The prerequisite lessons synthesize the named textbooks and standards; they are not claims made by the paper. Protocol behavior, model APIs, and benchmark leaderboards can change after publication, so verify current primary specifications separately.
...
- **Researcher:** derive At step t the policy samples a_t from pi_theta(a_t | x, thought_<t, action_<t, observation_<t), reconstruct the main comparison, identify three threats to validity, and preregister an ablation plus an external replication with explicit acceptance criteria.
