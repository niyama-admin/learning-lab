...
>
> **Why it is here:** instruction tuning and preference-based alignment. Read after the preceding items in the roadmap.

## One-picture map

```mermaid
flowchart LR
  A["base model"] --> B["demonstrations"]
  B["demonstrations"] --> C["reward model"]
  C["reward model"] --> D["optimized policy"]
  D["optimized policy"] --> E["human evaluation"]
```

## Tutorial 1 — intuitive understanding

Instead of only predicting text, the model is coached with examples and ranked responses so its behavior better matches human intent.
...
Separate supervised demonstrations, preference data, reward modeling, and policy optimization; monitor helpfulness, safety, regressions, and annotator disagreement.
...
Formalize the KL-regularized objective, reward-model identifiability, distribution shift, Goodhart effects, and the gap between stated and revealed preferences.
