# Memory from CPU Registers and Caches to GPU HBM

> **Primary sources:** AMD64 and Arm memory documentation; CUDA/HIP guides; Mei and Chu GPU memory microbenchmarking
>
...
| Lens | Answer |
|---|---|
| Guiding question | Why can the same arithmetic run hundreds of times slower depending on where data resides and how it is accessed? |
| Core model | Registers are explicitly named fast storage. Caches automatically retain recently used cache lines. Virtual memory translates process addresses through page tables and TLBs. CPU DRAM and discrete-GPU HBM/GDDR are normally separate physical pools connected by PCIe or a coherent fabric. Unified memory gives a common address-space abstraction but may migrate pages or access them remotely. |
| Critical comparison | CPUs invest in large coherent caches and latency reduction for a few complex threads. GPUs invest in bandwidth, many outstanding requests, coalescing, and latency hiding across many warps or wavefronts. Integrated systems may share physical memory but still have distinct caches and synchronization rules. |
| Formal lens | Effective time is not bytes/bandwidth alone: T = T_launch + T_transfer + T_translation + T_cache_miss + T_compute + T_sync. Arithmetic intensity I = operations/bytes helps determine whether a kernel is compute- or bandwidth-bound. |
| Practical lab | Measure array traversal with sequential, strided, and pointer-chasing patterns across working-set sizes. Plot latency and bandwidth, then identify likely cache, TLB, and DRAM regions without claiming exact geometry from one run. |

## Concept map

```mermaid
flowchart LR
 A["virtual address"] --> B["translation and TLB"] --> C["cache hierarchy"] --> D["DRAM or HBM"] --> E["CPU-GPU interconnect"]
 E -. measurement .-> A
```
...
+Think of a desk, nearby cabinet, building archive, and remote warehouse, with address forwarding and several clerks keeping copies consistent. Why can the same arithmetic run hundreds of times slower depending on where data resides and how it is accessed?

Registers are explicitly named fast storage. Caches automatically retain recently used cache lines. Virtual memory translates process addresses through page tables and TLBs. CPU DRAM and discrete-GPU HBM/GDDR are normally separate physical pools connected by PCIe or a coherent fabric. Unified memory gives a common address-space abstraction but may migrate pages or access them remotely.

Walk through virtual address -> translation and TLB -> cache hierarchy -> DRAM or HBM -> CPU-GPU interconnect. At the first stage, software expresses work. Intermediate stages translate, schedule, store, or move it. The final stage is observable only through architectural results and measurements. A faster middle stage may not improve the whole path when data movement or serialization dominates.

CPUs invest in large coherent caches and latency reduction for a few complex threads. GPUs invest in bandwidth, many outstanding requests, coalescing, and latency hiding across many warps or wavefronts. Integrated systems may share physical memory but still have distinct caches and synchronization rules.
...
Effective time is not bytes/bandwidth alone: T = T_launch + T_transfer + T_translation + T_cache_miss + T_compute + T_sync. Arithmetic intensity I = operations/bytes helps determine whether a kernel is compute- or bandwidth-bound.
...
Measure array traversal with sequential, strided, and pointer-chasing patterns across working-set sizes. Plot latency and bandwidth, then identify likely cache, TLB, and DRAM regions without claiming exact geometry from one run.
...
### Close-reading station 1: virtual address

Locate how the source defines **virtual address**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in virtual address -> translation and TLB -> cache hierarchy -> DRAM or HBM -> CPU-GPU interconnect. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 2: translation and TLB

Locate how the source defines **translation and TLB**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in virtual address -> translation and TLB -> cache hierarchy -> DRAM or HBM -> CPU-GPU interconnect. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 3: cache hierarchy

Locate how the source defines **cache hierarchy**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in virtual address -> translation and TLB -> cache hierarchy -> DRAM or HBM -> CPU-GPU interconnect. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 4: DRAM or HBM

Locate how the source defines **DRAM or HBM**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in virtual address -> translation and TLB -> cache hierarchy -> DRAM or HBM -> CPU-GPU interconnect. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 5: CPU-GPU interconnect

Locate how the source defines **CPU-GPU interconnect**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in virtual address -> translation and TLB -> cache hierarchy -> DRAM or HBM -> CPU-GPU interconnect. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.
