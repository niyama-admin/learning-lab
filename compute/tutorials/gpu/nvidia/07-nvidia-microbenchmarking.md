# NVIDIA Microbenchmarking: From Specification to Evidence

> **Primary sources:** Jia et al. Volta study; Mei and Chu memory-hierarchy study
>
...
| Lens | Answer |
|---|---|
| Guiding question | How can researchers infer undocumented latency, bandwidth, cache, and scheduler properties without confusing artifacts for architecture? |
| Core model | Pointer chasing exposes dependent-load latency; stride sweeps reveal capacity and line effects; throughput kernels create independent requests; instruction chains estimate latency; independent instructions estimate throughput. Generated machine code, compiler motion, clocks, warming, residency, and measurement overhead must be controlled. |
| Critical comparison | Vendor guides define supported semantics and limits but not every physical detail. Microbenchmarks add empirical implementation knowledge. Results for Volta, Maxwell, or a particular driver are not automatically true for Ampere, Hopper, Blackwell, or virtualized cloud GPUs. |
| Formal lens | Observed cycles O = target latency L + measurement overhead H + interference E. A valid design estimates H, bounds E through repetitions and controls, and varies one parameter whose predicted response differs between hypotheses. |
| Practical lab | Design a pointer-chasing kernel whose next address depends on the current load. Inspect generated code, subtract timer overhead, sweep working-set size, report distributions, and label cache-boundary conclusions as inferred. |

## Concept map

```mermaid
flowchart LR
 A["hypothesis"] --> B["minimal kernel"] --> C["controlled perturbation"] --> D["timing/counters"] --> E["competing explanation"]
 E -. measurement .-> A
```
...
+Think of learning a sealed building's layout by sending timed messengers along carefully chosen routes. How can researchers infer undocumented latency, bandwidth, cache, and scheduler properties without confusing artifacts for architecture?

Pointer chasing exposes dependent-load latency; stride sweeps reveal capacity and line effects; throughput kernels create independent requests; instruction chains estimate latency; independent instructions estimate throughput. Generated machine code, compiler motion, clocks, warming, residency, and measurement overhead must be controlled.

Walk through hypothesis -> minimal kernel -> controlled perturbation -> timing/counters -> competing explanation. At the first stage, software expresses work. Intermediate stages translate, schedule, store, or move it. The final stage is observable only through architectural results and measurements. A faster middle stage may not improve the whole path when data movement or serialization dominates.

Vendor guides define supported semantics and limits but not every physical detail. Microbenchmarks add empirical implementation knowledge. Results for Volta, Maxwell, or a particular driver are not automatically true for Ampere, Hopper, Blackwell, or virtualized cloud GPUs.
...
Observed cycles O = target latency L + measurement overhead H + interference E. A valid design estimates H, bounds E through repetitions and controls, and varies one parameter whose predicted response differs between hypotheses.
...
Design a pointer-chasing kernel whose next address depends on the current load. Inspect generated code, subtract timer overhead, sweep working-set size, report distributions, and label cache-boundary conclusions as inferred.
...
### Close-reading station 1: hypothesis

Locate how the source defines **hypothesis**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in hypothesis -> minimal kernel -> controlled perturbation -> timing/counters -> competing explanation. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 2: minimal kernel

Locate how the source defines **minimal kernel**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in hypothesis -> minimal kernel -> controlled perturbation -> timing/counters -> competing explanation. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 3: controlled perturbation

Locate how the source defines **controlled perturbation**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in hypothesis -> minimal kernel -> controlled perturbation -> timing/counters -> competing explanation. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 4: timing/counters

Locate how the source defines **timing/counters**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in hypothesis -> minimal kernel -> controlled perturbation -> timing/counters -> competing explanation. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 5: competing explanation

Locate how the source defines **competing explanation**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in hypothesis -> minimal kernel -> controlled perturbation -> timing/counters -> competing explanation. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.
