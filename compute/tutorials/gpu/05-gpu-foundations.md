# GPU Foundations: Throughput, SIMT/SIMD, and Memory

> **Primary sources:** Sun et al. CPU/GPU trends; Mei and Chu memory study; CUDA, HIP, and oneAPI guides
>
...
| Lens | Answer |
|---|---|
| Guiding question | Why are GPUs excellent for some workloads and disappointing for others? |
| Core model | A GPU devotes more area to replicated arithmetic and thread state than to sophisticated single-thread latency mechanisms. Thousands of logical threads are grouped for scheduling; groups share fast local memory and synchronize locally. Hardware swaps among ready groups to hide latency. Regular data-parallel work maps well; serial dependencies, irregular branches, tiny jobs, and random memory access do not. |
| Critical comparison | A CPU usually has fewer powerful cores, deep caches, branch prediction, speculative out-of-order execution, and strong latency performance. A GPU has many narrower lanes, high memory bandwidth, and massive concurrency. Neither is universally faster; parallel fraction, arithmetic intensity, transfer cost, and library quality decide. |
| Formal lens | A roofline upper bound is performance <= min(peak_compute, memory_bandwidth * arithmetic_intensity). End-to-end speedup must also include serial fraction and offload overhead; Amdahl's law bounds speedup by 1/((1-p)+p/s). |
| Practical lab | Implement vector addition and a reduction in a portable framework. Vary problem size and include allocation, transfer, launch, synchronization, and verification in separate timers. |

## Concept map

```mermaid
flowchart LR
 A["host program"] --> B["kernel launch"] --> C["thread groups"] --> D["parallel execution units"] --> E["device memory"]
 E -. measurement .-> A
```
...
+Think of a large kitchen organized around many identical meals, compared with a master chef optimized for unpredictable custom orders. Why are GPUs excellent for some workloads and disappointing for others?

A GPU devotes more area to replicated arithmetic and thread state than to sophisticated single-thread latency mechanisms. Thousands of logical threads are grouped for scheduling; groups share fast local memory and synchronize locally. Hardware swaps among ready groups to hide latency. Regular data-parallel work maps well; serial dependencies, irregular branches, tiny jobs, and random memory access do not.

Walk through host program -> kernel launch -> thread groups -> parallel execution units -> device memory. At the first stage, software expresses work. Intermediate stages translate, schedule, store, or move it. The final stage is observable only through architectural results and measurements. A faster middle stage may not improve the whole path when data movement or serialization dominates.
...
A roofline upper bound is performance <= min(peak_compute, memory_bandwidth * arithmetic_intensity). End-to-end speedup must also include serial fraction and offload overhead; Amdahl's law bounds speedup by 1/((1-p)+p/s).
...
Implement vector addition and a reduction in a portable framework. Vary problem size and include allocation, transfer, launch, synchronization, and verification in separate timers.
...
### Close-reading station 1: host program

Locate how the source defines **host program**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in host program -> kernel launch -> thread groups -> parallel execution units -> device memory. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 2: kernel launch

Locate how the source defines **kernel launch**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in host program -> kernel launch -> thread groups -> parallel execution units -> device memory. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 3: thread groups

Locate how the source defines **thread groups**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in host program -> kernel launch -> thread groups -> parallel execution units -> device memory. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 4: parallel execution units

Locate how the source defines **parallel execution units**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in host program -> kernel launch -> thread groups -> parallel execution units -> device memory. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.

### Close-reading station 5: device memory

Locate how the source defines **device memory**. Record whether it is guaranteed by an ISA/programming specification, described for one microarchitecture, or inferred experimentally. Connect it to the adjacent stages in host program -> kernel launch -> thread groups -> parallel execution units -> device memory. Then write one correctness test and one performance test. The correctness test must avoid timing assumptions; the performance test must record configuration and uncertainty.
